...
class Trainer(object):
    def __init__(self, args):
        ...
        self.criterion_KL = distiller.DistillKL(args.temperature).cuda()
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            ...
            optimizer.zero_grad()
            s_output, t_output, loss_distill = self.d_net(image)

            loss_seg = self.criterion(s_output, target)
            # loss_kl = self.criterion_KL(s_output, t_output)
            # loss = loss_seg + loss_distill.sum() * self.args.beta + loss_kl * self.args.alpha
            loss = loss_seg + loss_distill.sum() * self.args.beta
            loss.backward()
            optimizer.step()
    ...
    def validation(self, epoch):
        ...
        if new_pred > self.best_pred:
            ...
            self.saver.save_checkpoint({
                'epoch': epoch + 1,
                'state_dict': self.s_net.state_dict(),
                'best_pred': self.best_pred,
            }, is_best)
