...
parser.add_argument('--data_path', default='/dataset/ILSVRC/Data/CLS-LOC', type=str, help='path to dataset')
...
parser.add_argument('--is_checkpoint', default=0, type=int, help='is save check_point')
...
def main():
    ...
    criterion_CE = nn.CrossEntropyLoss().cuda()
    criterion_KL = distiller.DistillKL(args.temperature).cuda()
    optimizer = torch.optim.SGD(d_net.s_net.parameters(), args.lr, momentum=args.momentum, weight_decay=args.weight_decay, nesterov=True)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # train for one epoch
        train_with_distill(train_loader, d_net, optimizer, criterion_KL, epoch)
...
def train_with_distill(train_loader, d_net, optimizer, criterion_KL, epoch):
    d_net.train()
    d_net.s_net.train()
    d_net.t_net.train()
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        ...
        err1, err5 = accuracy(s_outputs.data, targets, topk=(1, 5))
        ...
        if i % args.print_freq == 0:
            print(
                'Train with distillation: [Epoch %d/%d][Batch %d/%d]\t Loss %.3f, Top 1-error %.3f, Top 5-error %.3f' %
